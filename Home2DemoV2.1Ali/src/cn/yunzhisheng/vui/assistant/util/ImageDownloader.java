...
public class ImageDownloader {
	...
	private Bitmap getBitmapFromCache(String url) {
		...
		Bitmap bitmap = BitmapFactory.decodeFile(mImgCacheDir.getAbsolutePath() + File.separator + fileName, options);
	 ...
	 * @param hideImageIfNull
	 * Hide the imageview if url is null or empty.
	 */
	...
	private class ImageRef {
		 ...
		 * from being garbage collected. This explains why we have to check that
		 * both the weak reference and the imageView are not null
		 */
	...
	private class BitmapDisplayer implements Runnable {
		...
		public BitmapDisplayer(Bitmap b, WeakReference<ImageView> i, int defaultResId, int scaleType) {
			...
			case SCALE_BY_IMAGEVIEW_WIDTH:
				...
				if (b!= null && w > 0 && b.getWidth() > 0) {
					bitmap = Bitmap.createScaledBitmap(b, w, (int) (w * ((float) b.getHeight() / b.getWidth())), false);
				}
				break;
			default:
				bitmap = b;
						...
						if (imageToLoad.imageViewReference != null) {
							...
							if (imageView != null) {
								...
								if (tag != null && ((String) tag).equals(imageToLoad.url)) {
									BitmapDisplayer bmpDisplayer = new BitmapDisplayer(bmp, imageToLoad.imageViewReference, mDefaultResId, imageToLoad.scaleType);

...
public abstract class SurfaceRunner extends SurfaceView implements SurfaceHolder.Callback {
	 ...
	 * {@link #postUpdate()}, just set a large delay. You may want to consider
	 * using 1000 -- i.e. one second -- to make sure you get a refresh at a
	 * decent interval.
	 */
	...
	private void stopRun() {
		...
		if (ticker != null && ticker.isAlive()) {
			if (onSurfaceThread()) ticker.kill();
			else ticker.killAndWait();
		}
		synchronized (surfaceHolder) {
			animTicker = null;
		}
	...
	private class ThreadTicker extends Thread implements Ticker {
		...
		public void killAndWait() {
			...
			if (Thread.currentThread() == this) throw new IllegalStateException("ThreadTicker.killAndWait()"
	...
	private class LoopTicker extends Thread implements Ticker {
		...
		public void killAndWait() {
			...
			if (Thread.currentThread() == this) throw new IllegalStateException("LoopTicker.killAndWait()"
																				+ " called from ticker thread");
		...
		public void run() {
			...
			msgHandler = new Handler() {
				...
				public void handleMessage(Message msg) {
					...
					case MSG_TICK:
						tick();
						if (!msgHandler.hasMessages(MSG_TICK)) msgHandler.sendEmptyMessageDelayed(
							MSG_TICK,
							animationDelay);
						break;
					case MSG_ABORT:
